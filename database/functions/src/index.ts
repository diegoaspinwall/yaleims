  ...
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      for (const bet of sortedBets) {
        ...
        const matchRef = admin.firestore().collection("matches_dha_testing").doc(bet.matchId);
...
export const addBet = functions.https.onRequest(async (req, res) => {
  corsHandler(req, res, async () => {
    const { email, matchId, betAmount, betOption, betOdds, away_college, home_college, sport, timestamp }: AddBetRequestBody = req.body;

    // Validation
    if (!email || !matchId || !betAmount || !betOption || !away_college || !home_college || !sport || !timestamp) {
...
export const deleteBet = functions.https.onRequest(async (req, res) => {
  ...
  corsHandler(req, res, async () => {
    if (!isDelBetRequestBody(req.query)) {
      return res.status(400).send("Required parameters are missing or incorrect.");
    }

    const { email, matchId, sport, betAmount, betOdds, betOption }: DelBetRequestBody = req.query;
    ...
    try {
      ...
      const bet = userData.bets.find((bet: any) => 
        bet.matchId === matchId &&
      ...
      if (!bet) {
        return res.status(404).send("Bet not found with the specified parameters");
      }
      ...
      const updatedBets = userData.bets.filter((bet: any) =>
        !(bet.matchId === matchId && 
          // bet.betAmount === betAmount &&
          bet.betAmount === betAmountNumber &&
          // bet.betOdds === betOdds &&
